...
public class MetricsAwareSampleProducer {
	...
	public static void main(String[] args) throws InterruptedException,
		...
		KinesisProducerConfiguration config = new KinesisProducerConfiguration()
				.setRecordMaxBufferedTime(3000).setMaxConnections(1)
				.setRequestTimeout(60000).setRegion(SampleProducer.REGION);
		...
		final FutureCallback<UserRecordResult> callback = new FutureCallback<UserRecordResult>() {
			...
			public void onFailure(Throwable t) {
				if (t instanceof UserRecordFailedException) {
					...
					log.error(String.format("Record failed to put - %s : %s",
				...
				while (true) {
					...
					log.info(String
							.format("Put %d of %d so far (%.2f %%), %d have completed (%.2f %%)",
									put, totalRecordsToPut, putPercent, done,
									donePercent));
					...
					try {
						for (Metric m : kinesisProducer.getMetrics(
								"UserRecordsPut", 5)) {
							// Metrics are emitted at different granularities,
							// here
							// we only look at the stream level metric, which
							// has a
							// single dimension of stream name.
							if (m.getDimensions().size() == 1
									&& m.getSampleCount() > 0) {
								log.info(String
										.format("(Sliding 5 seconds) Avg put rate: %.2f per sec, success rate: %.2f, failure rate: %.2f, total attemped: %d",
												m.getSum() / 5,
												m.getSum() / m.getSampleCount()
														* 100,
												(m.getSampleCount() - m
														.getSum())
														/ m.getSampleCount()
		...
		while (true) {
			...
			if (sequenceNumber.get() < totalRecordsToPut) {
				if (kinesisProducer.getOutstandingRecordsCount() < outstandingLimit) {
					ByteBuffer data = Utils.generateData(
		...
		for (Metric m : kinesisProducer
				.getMetrics("UserRecordsPerKinesisRecord")) {
			if (m.getDimensions().containsKey("ShardId")) {
				log.info(String
						.format("%.2f user records were aggregated into each Kinesis record on average for shard %s, for a total of %d Kinesis records.",
								m.getMean(), m.getDimensions().get("ShardId"),
